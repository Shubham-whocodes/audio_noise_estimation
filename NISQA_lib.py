...
#%% Evaluation 
def predict_mos(model, ds, bs, dev, num_workers=0,pred_col=''):    
    dl = DataLoader(ds,
                    batch_size=bs,
                    shuffle=False,
                    drop_last=False,
    ...
    ds.df[pred_col] = y_hat.astype(dtype=float)
    return y_hat, y
...
def calc_mapping(
    ...
    if dcon_db is not None:
        ...
        y = pd.Series(dfile_db[target_mos]).to_numpy()        
        y_hat = pd.Series(dfile_db[pred]).to_numpy()    
...
def eval_results(ground_truth,pred):
    y = torch.tensor(ground_truth).cpu().numpy()
    y_hat = torch.tensor(pred).cpu().numpy()      
    r = calc_eval_metrics(y, y_hat)
    return r
...
class biasLoss(object):
    ...
    def get_loss(self, yb, yb_hat, idx):
        ...
        if self.apply_bias_loss:
            ...
            # loss =  self._nan_mse(yb_hat, yb) + 0.90*listnet_loss(yb,yb_hat)
    ...
    def update_bias(self, y, y_hat):
        ...
        if self.apply_bias_loss:
            ...
            if self.do_update:
                if self.do_print:
                    print('--> bias updated')
                for db_name in self.Paths.unique():
                    
                    db_idx = (self.Paths==db_name).to_numpy().nonzero()
